...
// Demo version of PaymentGateways that doesn't interact with Firestore
function DemoPaymentGateways({
  paymentRequest,
  onPaymentComplete,
  onCancel,
}: {
  ...
  const processPayment = async (method: string) => {
    ...
    try {
      ...
      const mockTransactionId = `DEMO_TX${Date.now().toString().substring(5)}`;
      ...
      setError('Demo payment processing failed. Please try again.');
    } finally {
      ...
      {!isPaymentSuccessful ? (
        <>
          ...
          <div className="flex justify-between border-t pt-4 mt-4">
            ...
            <Button
              onClick={() => processPayment(selectedMethod as string)}
...
function PaymentDemoPage({}: PaymentDemoPageProps) {
  const toast = useToast();
  const [amount, setAmount] = useState('');
  const [description, setDescription] = useState('');
  const [isPaymentOpen, setIsPaymentOpen] = useState(false);
  const [demoPaymentRequests, setDemoPaymentRequests] = useState<PaymentRequest[]>([
    {
      amount: 1250,
      description: 'Monthly Maintenance Fee',
      fromApartmentId: 'apt-admin',
      fromApartmentName: 'Building Management',
      toApartmentId: 'apt-user',
      toApartmentName: 'Your Apartment',
  ...
  const handleCreatePayment = () => {
    ...
    const newPaymentRequest: PaymentRequest = {
      // Don't set an ID for demo requests - this prevents Firestore update attempts
      amount: amountValue,
      description,
      fromApartmentId: 'apt-user',
      fromApartmentName: 'Your Apartment',
      toApartmentId: 'apt-demo',
      toApartmentName: 'Demo Apartment',
    ...
    setDemoPaymentRequests(prev => [newPaymentRequest, ...prev]);
  };

  const handlePaymentComplete = (index: number) => {
    setDemoPaymentRequests(prev =>
      prev.map((request, i) => (i === index ? { ...request, status: 'paid' } : request))
    );
  };

  const handleDismiss = (index: number) => {
    setDemoPaymentRequests(prev => prev.filter((_, i) => i !== index));
  };
  ...
  return (
    <div className="container max-w-5xl py-6 md:py-10">
      <Card className="p-6">
        <CardHeader>
          <CardTitle className="text-2xl">Payment Integration Demo</CardTitle>
          <CardDescription>
            This page demonstrates how to use Google Pay and PhonePe payment integrations within
            your application.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-8">
              <Card>
                <CardHeader>
                  <CardTitle>Create a Test Payment</CardTitle>
                  <CardDescription>
                    Create a test payment to see the payment flow in action
                  </CardDescription>
                </CardHeader>
                ...
                <CardFooter>
                  <Button onClick={handleCreatePayment} disabled={!amount || !description}>
                    Test Payment Flow
                  </Button>
                </CardFooter>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Available payment methods that have been integrated
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex flex-col space-y-3">
                    ...
                    <div className="flex items-center gap-3 p-3 border rounded-lg">
                      ...
                      <div>
                        <div className="font-medium">UPI</div>
                        <div className="text-xs text-muted-foreground">All UPI apps supported</div>
                      </div>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                {demoPaymentRequests.length > 0 ? (
                  demoPaymentRequests.map((request, index) => (
                    <PaymentRequestCard
                      key={`demo-${index}`}
                      id={`demo-${index}`}
                      fromApartmentName={request.fromApartmentName}
                      amount={request.amount}
                      description={request.description}
                      createdAt={new Date().toISOString()}
                      dueDate={request.dueDate}
                      status={request.status}
                      onPaymentComplete={() => handlePaymentComplete(index)}
                      onDismiss={() => handleDismiss(index)}
                    />
                  ))
                ) : (
                  <div className="text-center p-8 bg-gray-50 rounded-lg border">
                    <p className="text-muted-foreground">No payment requests to display.</p>
                    <p className="text-sm text-muted-foreground mt-1">
                      Create a test payment to see it appear here.
                    </p>
                  </div>
                )}
              </CardContent>
